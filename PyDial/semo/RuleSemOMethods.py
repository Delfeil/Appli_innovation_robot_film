...
def parse_output(input_string):
    ...
    while output_scanner.cur[0] != tokenize.ENDMARKER:
        if output_scanner.cur[0] == tokenize.NAME or output_scanner.cur[1] in u'-_ éèêàîï':
...
class BasicTemplateRule(object):
    ...
    def __init__(self, scanner):
        ...
        if '))' in rule_act_str:
            logger.warning(u'Two )): ' + rule_act_str)
        if self.rule_act.act == 'badact':
            logger.error(u'Generated bac act rule: ' + rule_act_str)
        ...
        for i, word in enumerate(word_list):
            if word[0] == '$': # Variable $X
                if word not in non_term_map:
                    # logger.debug(u'%s not found in non_term_map %s.' % (word, str(non_term_map)))
                    num_missing += 1
                else:
    ...
    def match_act(self, act):
        ...
        for slot in self.rule_items:
            if slot[0] == '$':
                ...
                if len(value_list) > 1:
                    logger.error(u'Non-terminal %s is mapped to multiple values %s' % (slot, str(value_list)))
                dollar_rules[slot] = value_list[0]
            else:
                # Copy over rules that have a specified slot.
                rules[slot] = copy.deepcopy(self.rule_items[slot])

        logger.debug(u' rules: ' + str(rules))
        logger.debug(u'$rules: ' + str(dollar_rules))
...
class BasicTemplateFunction(object):
    ...
    def transform(self, inputs):
        ...
        logger.error(u'In function %s: No rule to transform inputs %s' % (self.function_name, str(inputs)))
...
class BasicTemplateFunctionRule(object):
    ...
    def __init__(self, scanner):
        ...
        #scanner.check_tokens([tokenize.STRING, '"'], 'Expected string output for template function rule.')
        self.output = scanner.cur[1].strip('\"').strip()
        ...
        for output_word in self.output:
            if output_word[0] == '$':
                if output_word not in self.input_map:
                    logger.error(u'Could not find variable %s' % output_word)
...
class BasicTemplateGenerator(object):
    ...
    def transform(self, sysAct):
        ...
        for rule in self.rules:
            logger.debug(u'Checking Rule %s' % unicode(rule))
            out, matches, missing, type_match, non_term_map = rule.generate(input_utt)
            if type_match > 0:
                logger.debug(u'Checking Rule %s: type_match=%d, missing=%d, matches=%d, output=%s' %
                             (unicode(rule), type_match, missing, matches, u' '.join(out)))
        ...
        if best_rule is not None:
            if best_missing > 0:
                logger.warning(u'While transforming %s, there were missing items.' % sysAct)
        else:
            logger.debug(u'No rule used.')
    ...
    def compute_ftn(self, input_words, non_term_map):
        ...
        for i, word in enumerate(input_words):
            if '%' not in word:
                continue
            logger.debug(u'Processing %s in %s...' % (word, str(input_words)))
            m = re.search(r'^([^\(\)]*)\((.*)\)(.*)$', word.strip())
            if m is None:
                logger.error(u'Parsing failed in %s' % word.strip())
            ftn_name = m.group(1)
            ...
            if '$' in ftn_name:
                ...
                if len(tokens) > 2:
                    logger.error(u'More than one underbar _ found in function name %s' % ftn_name)
                var = tokens[0][1:]
                if var not in non_term_map:
                    logger.error(u'Unable to find nonterminal %s in non terminal map.' % var)
                ftn_name = ftn_name.replace(var, non_term_map[var])

            # Processing function args.
            for j, arg in enumerate(ftn_args):
                if arg[0] == '%':
                    logger.error(u'% in function argument %s' % str(word))
                elif arg[0] == '$':
                    ftn_args[j] = non_term_map[arg]

            if ftn_name not in self.function_map:
                logger.error(u'Function name %s is not found.' % ftn_name)
            else:
...
class BasicSemO(SemOManager.SemO):
    ...
    def generate(self, act):
        if self.emphasis:
            logger.warning(u'Emphasis is not implemented.')
